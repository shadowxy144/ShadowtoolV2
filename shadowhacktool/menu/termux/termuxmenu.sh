...
pkg install nmap -y && pkg install cloudflared -y && pkg install python -y && pkg install whois
...
whois "$tarama" | tee whois_sonucu.txt #tarama sonucu başarılıysa whois_sonucu.txt adlı dosyaya kayıt eder
