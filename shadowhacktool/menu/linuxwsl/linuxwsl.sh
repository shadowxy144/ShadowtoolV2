...
sudo apt install nmap -y && sudo apt install cloudflare -y && sudo apt install python -y && sudo apt install whois
...
nmap "$ip" | tee "$anadizin/menu/logs/nmaplog.txt"
...
sudo nmap -sS -O "$ip2" | tee "$anadizin/menu/logs/nmaplog.txt"
...
sudo nmap -sS -O -sV "$ip3" | tee "$anadizin/menu/logs/nmaplog.txt"
...
cloudflared tunnel --url localhost:$port > "$anadizin/menu/logs/tunnel.txt" 2>&1 &

sleep 10

link=$(grep -o 'https://[a-zA-Z0-9-]*\.trycloudflare\.com' "$anadizin/menu/logs/tunnel.txt")
...
whois "$tarama" | tee "$anadizin/menu/logs/whois_sonucu.txt"
