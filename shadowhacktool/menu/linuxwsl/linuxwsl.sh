...
nmap "$ip" | tee "$anadizin/logs/nmaplog.txt"
...
sudo nmap -sS -O "$ip2" | tee "$anadizin/logs/nmaplog.txt"
...
sudo nmap -sS -O -sV "$ip3" | tee "$anadizin/logs/nmaplog.txt"
...
cloudflared tunnel --url localhost:$port > "$anadizin/logs/tunnel.txt" 2>&1 &

sleep 10

link=$(grep -o 'https://[a-zA-Z0-9-]*\.trycloudflare\.com' "$anadizin/logs/tunnel.txt")
